...
class BusinessRegistration(models.Model):
    ...
    total_shares = models.IntegerField(default=1000000)  # Total shares available
    available_shares = models.IntegerField(default=1000000)  # Shares available for investment
    share_price = models.DecimalField(max_digits=10, decimal_places=2, default=10.00)  # Price per share

    def calculate_shares_and_price(self):
        # Example calculation logic:
        # Let's say we want to set the share price to be 10% of the annual revenue divided by total shares.
        self.share_price = (self.annual_revenue * Decimal('0.10')) / Decimal(self.total_shares)
        self.available_shares = self.total_shares  # Initially, all shares are available

    def save(self, *args, **kwargs):
        # Calculate share price and number of shares before saving
        self.calculate_shares_and_price()
        super().save(*args, **kwargs)

    def clean(self):
        """Custom validation to ensure fields are valid before saving."""
        if self.annual_revenue < 0:
            raise ValidationError("Annual revenue must be a positive number.")
        if self.total_shares <= 0:
            raise ValidationError("Total shares must be greater than zero.")
        if self.available_shares < 0:
            raise ValidationError("Available shares cannot be negative.")

    def invest(self, investor, shares):
        if shares > self.available_shares:
            raise ValueError("Not enough shares available for investment.")
        
        investment_amount = shares * self.share_price
        Investment.objects.create(business=self, investor=investor, shares=shares, amount=investment_amount).save()
...
class Investment(models.Model):
    ...
    investor = models.ForeignKey('CustomUser', on_delete=models.CASCADE)  # Use string reference
    shares = models.IntegerField()
    amount = models.DecimalField(max_digits=10, decimal_places=2)

    def __str__(self):
        return f"{self.investor.email} invested in {self.business.business_name}"
# class BusinessRegistration(models.Model):
#     business_name = models.CharField(max_length=255)
